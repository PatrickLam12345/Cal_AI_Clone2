...
router.get("/search", async (req, res) => {
  try {
    ...
    const USDA_API_KEY = req.app.locals.usdaApiKey.value();

    // Try Foundation first
    const foundation = await searchRaw(q, { page, dataType: "Foundation", pageSize, apiKey: USDA_API_KEY });
    ...
    if (foods.length === 0) {
      const legacy = await searchRaw(q, { page, dataType: "SR Legacy", pageSize, apiKey: USDA_API_KEY });
...
router.get("/detail/:fdcId", async (req, res) => {
  try {
    ...
    // Get API key from app locals (set in main index.js)
    const USDA_API_KEY = req.app.locals.usdaApiKey.value();
    const detail = await getDetail(fdcId, USDA_API_KEY);
    res.json(detail);
  } catch (e) {
...
async function fetchJson(url, opts = {}, ms = 8000) {
  ...
  try {
    ...
    const r = await globalThis.fetch(url, { ...opts, signal: ac.signal });
    return r;
  } finally {
    clearTimeout(t);
  }
...
async function searchRaw(query, { page = 1, dataType, pageSize = 25, apiKey } = {}) {
...
async function getDetail(fdcId, apiKey) {
